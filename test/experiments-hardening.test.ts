...
const clientResourceCsv = "timestamp,cpu_user,cpu_system,rss,cpu_user_delta_us,cpu_system_delta_us,wall_delta_us,cpu_utilization_percent\n1,10,5,1024,1,1,100,2\n";
const canonicalW1WindowId = "https://rsp.js/w1:[1787711296979,1787711356979)";
const notificationServiceResourceCsv = "timestamp,cpu_user_jiffies,cpu_system_jiffies,rss_bytes,wall_delta_ms,cpu_utilization_percent\n1,10,5,1024,,,\n2,11,5,1024,100,1\n";
const heimdallServiceResourceCsv = "timestamp_epoch_ms,cpu_user_us,cpu_system_us,rss_bytes,heap_total_bytes,heap_used_bytes,external_bytes,cpu_user_delta_us,cpu_system_delta_us,wall_delta_us,cpu_utilization_percent\n1,10,5,1024,512,256,128,,,,\n2,11,5,1024,512,256,128,1,0,100,1\n";
...
function writeHeimdallFixture(directory: string, creations: number, clientHashes?: string[], staged = false, clientCount = 2): void {
    ...
    fs.writeFileSync(path.join(directory, "service-resource.csv"), heimdallServiceResourceCsv); fs.mkdirSync(path.join(directory, "service"));
...
function writeNotificationStagedFixture(directory: string): void {
    ...
    fs.writeFileSync(path.join(directory, "client-host-resource.csv"), "timestamp,cpu_user,cpu_nice,cpu_system,cpu_idle,cpu_iowait,cpu_irq,cpu_softirq,cpu_steal\n1,1,1,1,1,1,1,1,1\n");
    fs.writeFileSync(path.join(directory, "service-resource.csv"), notificationServiceResourceCsv);
...
function writeWithoutAggregatorFixture(directory: string): void {
    writeNotificationStagedFixture(directory);
    fs.writeFileSync(path.join(directory, "metadata.json"), "{}\n");
    for (let client = 0; client < 2; client += 1) {
        fs.writeFileSync(path.join(directory, `client-${client}-operations.csv`), `client_id,query_id,operation,window_id,duration_ms,start_monotonic_ns,end_monotonic_ns\n${client},q,stream_discovery,,,1,2\n${client},q,stream_subscription,,,2,3\n${client},q,parsing_timestamp_extraction,,,3,4\n${client},q,rsp_insertion,"${canonicalW1WindowId}",,4,5\n${client},q,r2r_first_result,"${canonicalW1WindowId}",,5,6\n${client},q,registration_to_first_result,,1,1,2\n`);
    }
}
...
`, "service-monitor-harness"], { cwd: path.resolve(__dirname, "..") }).toString();
    ...
    const rspPreflightStart = runnerSource.indexOf("heimdall_rsp_js_preflight_command() {");
    const rspPreflightEnd = runnerSource.indexOf("\n}\nservice_launch_command()", rspPreflightStart);
    assert.ok(rspPreflightStart >= 0 && rspPreflightEnd > rspPreflightStart, "Heimdall RSP-JS preflight command is present");
    const rspPreflightFunction = runnerSource.slice(rspPreflightStart, rspPreflightEnd + 2);
    const rspPreflightHarness = execFileSync("bash", ["-c", `set -euo pipefail
root=$(mktemp -d); trap 'rm -rf "$root"' EXIT
rsp_js_path=$(cd ../RSP-JS && pwd -P); rsp_js_sha=$(git -C "$rsp_js_path" rev-parse HEAD); heimdall_path="$root/heimdall"; mkdir -p "$heimdall_path/node_modules"; ln -s "$rsp_js_path" "$heimdall_path/node_modules/rsp-js"
shell_quote() { printf "'%s'" "$1"; }; remote_path_expression() { shell_quote "$1"; }
${rspPreflightFunction}
correct_command=$(heimdall_rsp_js_preflight_command); bash -c "$correct_command"
mkdir -p "$root/old-rsp/dist"; printf '{"name":"rsp-js","main":"dist/index.js"}\\n' > "$root/old-rsp/package.json"; printf 'module.exports = {}\\n' > "$root/old-rsp/dist/index.js"; rm "$heimdall_path/node_modules/rsp-js"; ln -s "$root/old-rsp" "$heimdall_path/node_modules/rsp-js"
wrong_command=$(heimdall_rsp_js_preflight_command); if bash -c "$wrong_command" >"$root/wrong.out" 2>"$root/wrong.err"; then exit 1; fi; grep -F 'resolved rsp-js is outside the frozen checkout' "$root/wrong.err" >/dev/null`], { cwd: path.resolve(__dirname, "..") }).toString();
    assert.strictEqual(rspPreflightHarness, "", "Heimdall preflight accepts the frozen RSP-JS resolution and rejects another installation");
    const serviceLaunchHarness = execFileSync("bash", ["-c", `set -euo pipefail
root=$(mktemp -d); trap 'rm -rf "$root"' EXIT; mkdir -p "$root/bin"
ln -s /usr/bin/true "$root/bin/setsid"
printf '%s\\n' '#!/usr/bin/env bash' 'if test "$NODE_DISCOVERY_MODE" = child && test "$4" = 4242; then printf node; elif test "$NODE_DISCOVERY_MODE" = leader; then printf node; else printf bash; fi' > "$root/bin/ps"
printf '%s\\n' '#!/usr/bin/env bash' 'test "$NODE_DISCOVERY_MODE" = child && printf "4242\\n"' > "$root/bin/pgrep"
printf '%s\\n' '#!/usr/bin/env bash' 'exit 0' > "$root/bin/sleep"; chmod +x "$root/bin/ps" "$root/bin/pgrep" "$root/bin/sleep"; export PATH="$root/bin:$PATH"
service_results_root="$root"; service_resource_header=x; service_start_exec_quoted="'true'"; remote_path_expression() { printf "'%s'" "$1"; }
...
NODE_DISCOVERY_MODE=none; export NODE_DISCOVERY_MODE; missing_command=$(service_launch_command iteration-missing); if bash -c "$missing_command" >"$root/missing.out" 2>"$root/missing.err"; then exit 1; fi; grep -F 'service launch: Node PID was not discovered within 1 seconds' "$root/missing.err" >/dev/null`, "service-launch-harness"], { cwd: path.resolve(__dirname, ".."), stdio: "ignore" });
    ...
    const clientArtifacts = runnerSource.indexOf('experiment_scp_from "$client_host" "$evaluation_path/$iteration_dir"');
    const serviceArtifacts = runnerSource.indexOf('experiment_scp_from "$service_host" "$service_iteration_dir"');
    const finalFailure = runnerSource.indexOf('"$infrastructure_failed" != true ]] || exit 1');
    assert.ok(monitorWait >= 0 && clientArtifacts > monitorWait && serviceArtifacts > clientArtifacts && finalFailure > serviceArtifacts, "monitor failure is recorded before artifact collection and final failure reporting");
    const monitorCollectionHarness = execFileSync("bash", ["-c", `set -euo pipefail
root=$(mktemp -d); trap 'rm -rf "$root"' EXIT; infrastructure_failed=false
wait() { return 1; }
if ! wait 42; then infrastructure_failed=true; fi
touch "$root/client-artifacts" "$root/network-artifacts" "$root/service-artifacts"
test -e "$root/client-artifacts" && test -e "$root/network-artifacts" && test -e "$root/service-artifacts"
set +e; ( [[ "$infrastructure_failed" != true ]] || exit 1 ); status=$?; set -e; test "$status" -eq 1`], { cwd: path.resolve(__dirname, "..") }).toString();
    assert.strictEqual(monitorCollectionHarness, "", "monitor failure remains nonzero after collection artifacts are produced");
    ...
    const notificationDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "notification-staged-")); writeNotificationStagedFixture(notificationDirectory); assert.deepStrictEqual(validateMultiClientRepetition(notificationDirectory, "notification-aggregator", 2), { valid: true, errors: [] }, "staged notification join evidence passes with valid client and service resource samples"); fs.writeFileSync(path.join(notificationDirectory, "client-1-resource.csv"), "timestamp\n"); assert.ok(!validateMultiClientRepetition(notificationDirectory, "notification-aggregator", 2).valid, "Notification Aggregator fails when one of two client resource files is header-only"); fs.writeFileSync(path.join(notificationDirectory, "client-1-resource.csv"), clientResourceCsv); fs.writeFileSync(path.join(notificationDirectory, "service-resource.csv"), "timestamp\n"); assert.ok(validateMultiClientRepetition(notificationDirectory, "notification-aggregator", 2).errors.some((error) => error.includes("service-resource.csv has no service resource samples")), "Notification Aggregator service-resource header-only validation remains enforced separately"); fs.writeFileSync(path.join(notificationDirectory, "service-resource.csv"), "timestamp,cpu_user_jiffies,cpu_system_jiffies,rss_bytes,wall_delta_ms,cpu_utilization_percent\n1,10,5,1024,,,\n"); assert.ok(validateMultiClientRepetition(notificationDirectory, "notification-aggregator", 2).errors.some((error) => error.includes("service-resource.csv has no usable CPU/RSS sample")), "Notification Aggregator requires a usable service CPU sample"); fs.writeFileSync(path.join(notificationDirectory, "service-resource.csv"), notificationServiceResourceCsv); fs.writeFileSync(path.join(notificationDirectory, "client-0-operations.csv"), fs.readFileSync(path.join(notificationDirectory, "client-0-operations.csv"), "utf8").split(canonicalW1WindowId).join("https://rsp.js/w2:[1787711296979,1787711356979)")); assert.ok(validateMultiClientRepetition(notificationDirectory, "notification-aggregator", 2).errors.some((error) => error.includes("canonical W1")), "W2 does not satisfy per-client canonical W1 validation"); fs.rmSync(notificationDirectory, { recursive: true, force: true });
    const missingRssDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "client-resource-rss-")); writeWithoutAggregatorFixture(missingRssDirectory); fs.writeFileSync(path.join(missingRssDirectory, "client-0-resource.csv"), clientResourceCsv.replace(",rss", "")); assert.ok(validateMultiClientRepetition(missingRssDirectory, "without-aggregator", 2).errors.some((error) => error.includes("missing rss")), "without-aggregator requires client RSS"); fs.rmSync(missingRssDirectory, { recursive: true, force: true });
    const missingCpuDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "client-resource-cpu-")); writeWithoutAggregatorFixture(missingCpuDirectory); fs.writeFileSync(path.join(missingCpuDirectory, "client-0-resource.csv"), clientResourceCsv.replace(",cpu_utilization_percent", "")); assert.ok(validateMultiClientRepetition(missingCpuDirectory, "without-aggregator", 2).errors.some((error) => error.includes("missing cpu_utilization_percent")), "without-aggregator requires client CPU utilization"); fs.rmSync(missingCpuDirectory, { recursive: true, force: true });
    const withoutAggregatorDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "without-aggregator-resource-")); writeWithoutAggregatorFixture(withoutAggregatorDirectory); assert.deepStrictEqual(validateMultiClientRepetition(withoutAggregatorDirectory, "without-aggregator", 2), { valid: true, errors: [] }, "without-aggregator validates client resource samples"); fs.rmSync(withoutAggregatorDirectory, { recursive: true, force: true });
    const heimdallResourceDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "heimdall-resource-")); writeHeimdallFixture(heimdallResourceDirectory, 1); assert.deepStrictEqual(validateMultiClientRepetition(heimdallResourceDirectory, "heimdall", 2), { valid: true, errors: [] }, "Heimdall validates client resources, derived service CPU/RSS, and its canonical service W1"); fs.writeFileSync(path.join(heimdallResourceDirectory, "service", "window-processing.csv"), "operation,window_id\nr2r_first_result,\"https://rsp.js/w2:[1787711296979,1787711356979)\"\n"); assert.ok(validateMultiClientRepetition(heimdallResourceDirectory, "heimdall", 2).errors.some((error) => error.includes("canonical W1")), "W2 does not satisfy Heimdall service-side canonical W1 validation"); fs.writeFileSync(path.join(heimdallResourceDirectory, "service", "window-processing.csv"), `operation,window_id\nr2r_first_result,"${canonicalW1WindowId}"\n`); fs.writeFileSync(path.join(heimdallResourceDirectory, "service-resource.csv"), "timestamp_epoch_ms\n"); assert.ok(validateMultiClientRepetition(heimdallResourceDirectory, "heimdall", 2).errors.some((error) => error.includes("requires at least two resource samples")), "Heimdall header-only service resource evidence fails"); fs.writeFileSync(path.join(heimdallResourceDirectory, "service-resource.csv"), "timestamp_epoch_ms,cpu_user_us,cpu_system_us,rss_bytes,heap_total_bytes,heap_used_bytes,external_bytes\n1,10,5,1024,512,256,128\n2,11,5,1024,512,256,128\n"); assert.ok(validateMultiClientRepetition(heimdallResourceDirectory, "heimdall", 2).errors.some((error) => error.includes("missing cpu_utilization_percent")), "Heimdall cumulative-only service CPU evidence fails"); fs.writeFileSync(path.join(heimdallResourceDirectory, "service-resource.csv"), heimdallServiceResourceCsv); fs.writeFileSync(path.join(heimdallResourceDirectory, "client-1-resource.csv"), "timestamp\n"); assert.ok(!validateMultiClientRepetition(heimdallResourceDirectory, "heimdall", 2).valid, "Heimdall fails on header-only client resource samples"); fs.rmSync(heimdallResourceDirectory, { recursive: true, force: true });
}
