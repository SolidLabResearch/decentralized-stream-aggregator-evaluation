...
assert.match(normalizedDryRun, /expected-shas: .*heimdall=32c9e3adc254cfd6f79eea71ab121b7bc344ae86/);
...
assert.match(runnerSource, /wait_for_replay_measurement_window\(\)/);
...
assert.match(runnerSource, /replay_deadline=\$\(\(SECONDS \+ duration\)\)/);
...
assert.ok(runnerSource.indexOf('wait_for_replay_measurement_window "$replay_deadline"') > runnerSource.indexOf('experiment_ssh "$replayer_host" "$replayer_launch_command"'));
...
assert.match(runnerSource, /all_client_first_result_markers_ready_command/);
assert.match(runnerSource, /capture_network_snapshots end[\s\S]*stop_replayer_process_group/);
