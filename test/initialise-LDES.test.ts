...
assert.match(runnerSource, /query_registration >= 1/);
assert.match(runnerSource, /stream_subscription >= 3/);
assert.match(runnerSource, /initialization\.csv/);
assert.match(runnerSource, /test -f %s/);
assert.match(runnerSource, /wait_for_command "Heimdall query\/subscription readiness"/);
assert.match(runnerSource, /wait_for_command "first completed Heimdall window evaluation"/);
assert.match(runnerSource, /EXPERIMENT_STOP_AFTER_FIRST_WINDOW:-false/);
assert.match(runnerSource, /window_query_processing/);
