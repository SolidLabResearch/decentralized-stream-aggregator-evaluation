...
function processClientData(clientNumber: number): ClientResults {
    ...
    // Collect all add_event and fetch_event values across all iterations for overall summary
...
function generateMarkdownReport(allResults: ClientResults[]): string {
    let markdown = `# Without Aggregator Approach - Analysis Report\n\n`;
    markdown += `Generated on: ${new Date().toISOString()}\n\n`;

    // Summary Table
    markdown += `## Summary Table\n\n`;
    markdown += `| Client | Add Event Mean | Add Event SD | Fetch Event Mean | Fetch Event SD | Fetch Event Sum | Preprocess Mean | Preprocess SD | Preprocess Sum |\n`;
    markdown += `|--------|----------------|--------------|------------------|----------------|------------------|-----------------|---------------|----------------|\n`;

    allResults.forEach(result => {
        markdown += `| ${result.client} | ${result.summary.add_event_mean.toFixed(2)} | ${result.summary.add_event_sd.toFixed(2)} | ${result.summary.fetch_event_mean.toFixed(2)} | ${result.summary.fetch_event_sd.toFixed(2)} | ${result.summary.fetch_event_sum.toFixed(2)} | ${result.summary.preprocess_mean.toFixed(2)} | ${result.summary.preprocess_sd.toFixed(2)} | ${result.summary.preprocess_sum.toFixed(2)} |\n`;
    });
    ...
    allResults.forEach(clientResult => {
        markdown += `### Client ${clientResult.client}\n\n`;
        markdown += `| Iteration | Add Event Mean | Add Event SD | Fetch Event Mean | Fetch Event SD | Fetch Event Sum | Preprocess Mean | Preprocess SD | Preprocess Sum |\n`;
        markdown += `|-----------|----------------|--------------|------------------|----------------|------------------|-----------------|---------------|----------------|\n`;

        clientResult.iterations.forEach(iter => {
            markdown += `| ${iter.iteration} | ${iter.add_event_mean.toFixed(2)} | ${iter.add_event_sd.toFixed(2)} | ${iter.fetch_event_mean.toFixed(2)} | ${iter.fetch_event_sd.toFixed(2)} | ${iter.fetch_event_sum.toFixed(2)} | ${iter.preprocess_mean.toFixed(2)} | ${iter.preprocess_sd.toFixed(2)} | ${iter.preprocess_sum.toFixed(2)} |\n`;
        });

        markdown += `\n**Summary for Client ${clientResult.client}:**\n`;
...
function main() {
    console.log('Starting analysis of Without Aggregator Approach data...');
    ...
    // Write to file
    const outputPath = path.join(__dirname, '../../without-aggregator-analysis-report.md');
    fs.writeFileSync(outputPath, markdownReport);

    console.log(`Analysis complete! Report saved to: ${outputPath}`);
    console.log('\nSummary:');
    allResults.forEach(result => {
        console.log(`Client ${result.client}: ${result.iterations.length} iterations processed`);
    });
}
