...
evaluation_sha="$(git rev-parse HEAD)"; heimdall_sha="5fca4fb5b35f5626e1cd382667920552f03d550a"; rsp_js_sha="45112d2955b99796d234747db34bd6804939e69a"
...
case "$approach" in
  ...
  heimdall)
    ...
    heimdall_solid_start="${HEIMDALL_SOLID_START_COMMAND:-<HEIMDALL_SOLID_START_COMMAND is required>}"
    heimdall_solid_ready="${HEIMDALL_SOLID_READY_COMMAND:-<HEIMDALL_SOLID_READY_COMMAND is required>}"
...
print_plan() {
  ...
  if [[ "$service_host" == "none" ]]; then echo "service: none"
    ...
    echo "heimdall-local-solid: $(experiment_ssh_preview "$service_host" "$heimdall_solid_start")"
    echo "heimdall-local-solid-readiness: $(experiment_ssh_preview "$service_host" "$heimdall_solid_ready")"
    echo "heimdall: $(experiment_ssh_preview "$service_host" "$service_start")"
...
if [[ "$mode" == "--preflight" ]]; then
  ...
  if [[ "$approach" == "heimdall" ]]; then
    for command in "$heimdall_solid_start" "$heimdall_solid_ready" "$service_start"; do if command_required "$command"; then echo "CONFIG MISSING: ${command#<}" >&2; failures=1; fi; done
  elif [[ "$service_host" != "none" ]] && command_required "$service_start"; then echo "CONFIG MISSING: ${service_start#<}" >&2; failures=1; fi
  remote_check "REPLAYER reachable/path/SHA/node" "$replayer_host" "test -d \"$replayer_path\" && git -C \"$replayer_path\" rev-parse HEAD && command -v node && node --version" || failures=1
...
for command in "$solid_initialize" "$solid_cleanup" "$replayer_start" "$service_start" "${heimdall_solid_start:-:}" "${heimdall_solid_ready:-:}"; do if command_required "$command"; then echo "Set ${command#<} before running." >&2; exit 2; fi; done
client_pid_file="$evaluation_path/.4hz-$approach-launcher.pid"
heimdall_solid_pid_file="$evaluation_path/.4hz-heimdall-solid-$run_id.pid"
heimdall_pid_file="$evaluation_path/.4hz-heimdall-$run_id.pid"
cleanup() {
  ...
  if [[ "$approach" == "heimdall" ]]; then
    experiment_ssh "$service_host" "for pidfile in \"$heimdall_pid_file\" \"$heimdall_solid_pid_file\"; do if test -f \"\$pidfile\"; then kill -TERM \$(cat \"\$pidfile\") 2>/dev/null || true; rm -f \"\$pidfile\"; fi; done" || true
  fi
}
...
for iteration in $(seq 1 "$iterations"); do
  ...
  service_pid=""; heimdall_solid_pid=""
  if [[ "$service_host" != "none" ]]; then
    if [[ "$approach" == "heimdall" ]]; then
      experiment_ssh "$service_host" "mkdir -p \"$evaluation_path/$iteration_dir/service\" && ( $heimdall_solid_start ) > \"$evaluation_path/$iteration_dir/service/heimdall-solid.log\" 2>&1 & echo \$! > \"$heimdall_solid_pid_file\"; wait \$(cat \"$heimdall_solid_pid_file\")" >"$root/$iteration_dir/heimdall-solid-launcher.log" 2>&1 & heimdall_solid_pid=$!
      wait_for_command "Heimdall-local Solid server" "$heimdall_solid_ready" "${HEIMDALL_SOLID_READY_TIMEOUT_SECONDS:-30}"
      experiment_ssh "$service_host" "( export HEIMDALL_RESULTS_DIR=\"$evaluation_path/$iteration_dir/service\" HEIMDALL_RUN_ID='$run_id-$(printf '%02d' "$iteration")' HEIMDALL_APPROACH=heimdall HEIMDALL_RSP_JS_PATH=\"$rsp_js_path\" HEIMDALL_RESOURCE_INTERVAL_MS='$(read_config 'c.experiment.resourceSamplingIntervalMs')'; $service_start ) > \"$evaluation_path/$iteration_dir/service/heimdall.log\" 2>&1 & echo \$! > \"$heimdall_pid_file\"; wait \$(cat \"$heimdall_pid_file\")" >"$root/$iteration_dir/$approach.log" 2>&1 & service_pid=$!
  ...
  pids=("$client_pid" "$replayer_pid"); if [[ -n "$service_pid" ]]; then pids+=("$service_pid"); fi; if [[ -n "$heimdall_solid_pid" ]]; then pids+=("$heimdall_solid_pid"); fi
