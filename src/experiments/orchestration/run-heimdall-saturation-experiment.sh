...
output_root="${EXPERIMENT_OUTPUT_ROOT:-results/4hz/heimdall-saturation/${mode}/clients-${count}/run-${run_id}}"
if [[ "$action" == "--preflight" ]]; then
  ...
  E4_READ_ONLY_PREFLIGHT=true EXPERIMENT_CONFIG_PATH="$config_path" EXPERIMENT_CONFIG_OVERRIDES="$overrides" EXPERIMENT_RUN_ID="$run_id" EXPERIMENT_OUTPUT_ROOT="$output_root" CLIENT_READY_TIMEOUT_SECONDS="${SATURATION_CLIENT_READY_TIMEOUT_SECONDS:-300}" "$root/src/experiments/orchestration/run-experiment.sh" heimdall --preflight
  exit $?
fi
set +e
