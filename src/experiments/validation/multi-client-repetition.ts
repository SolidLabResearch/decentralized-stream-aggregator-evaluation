...
function isCanonicalW1WindowId(windowId: string | undefined): boolean {
    const match = /^https:\/\/rsp\.js\/w1:\[(\d+),(\d+)\)$/.exec(windowId || "");
    return Boolean(match && BigInt(match[1]) < BigInt(match[2]));
}
...
export function validateMultiClientRepetition(iterationDirectory: string, approach: string, clientCount: number): Validation {
    ...
    for (let client = 0; client < clientCount; client += 1) {
        ...
        if (approach !== "heimdall" && !operationRows.some(row => row.operation === "r2r_first_result" && isCanonicalW1WindowId(row.window_id))) errors.push(`missing canonical W1 r2r_first_result for client ${client}`);
    }
    ...
    if (approach === "notification-aggregator") {
        ...
        if (fs.existsSync(serviceResource)) {
            const serviceRows = rows(serviceResource);
            const serviceHeader = fs.readFileSync(serviceResource, "utf8").split(/\r?\n/, 1)[0].split(",");
            for (const column of ["timestamp", "cpu_user_jiffies", "cpu_system_jiffies", "rss_bytes", "wall_delta_ms", "cpu_utilization_percent"]) if (!serviceHeader.includes(column)) errors.push(`missing ${column} in ${serviceResource}`);
            if (!serviceRows.length) errors.push(`${serviceResource} has no service resource samples`);
            if (serviceRows.length && !serviceRows.some((row) => finiteNonNegative(row.rss_bytes) && finiteNonNegative(row.cpu_utilization_percent))) errors.push(`${serviceResource} has no usable CPU/RSS sample`);
        }
    }
    if (approach === "heimdall") {
        const metadataPath = path.join(iterationDirectory, "metadata.json");
