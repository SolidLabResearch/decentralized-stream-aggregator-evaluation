...
export async function initializeWithoutAggregatorClients(number_of_clients: number) {
    ...
    for (let i = 0; i < number_of_clients; i++) {
        clients.push(without_aggregator_client(number_of_clients));
    }
    await Promise.all(clients);
}

async function without_aggregator_client(number_of_clients: number) {
        ...
        if (request.method === "POST") {
            ...
            request.on("end", async () => {
                try {
                    ...
                    fs.appendFileSync(`without-aggregator-${number_of_clients}-clients.csv`, `time_to_fetch_notification,${time_after_fetching - time_before_fetching}\n`);
    ...
    for (const stream of stream_array) {
        ...
        if (if_subscription_is_true) {
            ...
            fs.appendFileSync(`without-aggregator-${number_of_clients}-clients.csv`, `time_to_subscribe,${time_after_subscribing - time_before_subscribing}\n`);
        }
    ...
    subscribe_to_results(rsp_emitter, time_to_start_subscribing_results, number_of_clients);
}
...
export function subscribe_to_results(rsp_emitter: any, time_to_start_subscribing_results: number, number_of_clients: number) {
    const listener = (event: any) => {
        let iterable = event.bindings.values();
        for (let item of iterable) {
            const time_recieved_aggregated_result = Date.now();
            fs.appendFileSync(`without-aggregator-${number_of_clients}-clients.csv`, `time_received_aggregation_event,${time_recieved_aggregated_result - time_to_start_subscribing_results}\n`);
            time_to_start_subscribing_results = time_recieved_aggregated_result;
            fs.appendFileSync(`result-${number_of_clients}-clients.csv`, `${item.value}\n`);
        }
    }
