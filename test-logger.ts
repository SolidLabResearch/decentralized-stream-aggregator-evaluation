...
const logger = winston.createLogger({
    ...
    transports: [
        ...
        new winston.transports.File({
            filename: 'app.log', // File path where logs will be stored
...
import { add_event_to_rsp_engine, epoch, record_usage, insertion_sort } from "./evaluations/e-health/increasing-window-sizes/Util";
...
export async function query1sec() {
    ...
    let from_date = new Date(to_date.getTime() - 1);
    let ldes = new LDESinLDP(ldes_location, communication);
    logger.info(`Preprocessing of the events has started.`);
    ...
    stream.on("end", async () => {
        logger.info(`Preprocessing of the events has ended.`);
    ...
    emitter.on('RStream', async (data: any) => {
        ...
        if (query_registered_time !== null && file_streamer_done_time !== null && first_message_arrival_time !== null && !has_been_written) {
            fs.appendFileSync('query_latency_noagg.csv', `1,${(file_streamer_done_time - query_registered_time) / 1000},${(first_message_arrival_time - file_streamer_done_time) / 1000}\n`);
