...
const N3 = require('n3');
import fs from 'fs';
let ldes_location_1sec = 'http://n061-14a.wall2.ilabt.iminds.be:3000/participant6/bvp-1min/';
...
export async function query1sec() {
    ...
    FROM NAMED WINDOW :w1 ON STREAM <${ldes_location_1sec}> [RANGE ${range_to_read} STEP 10]
    WHERE {
        WINDOW :w1 {
            ?s saref:hasValue ?o .
