...
export async function query180sec() {
    ...
    FROM NAMED WINDOW :w1 ON STREAM <${ldes_location}> [RANGE 1800 STEP 10]
    WHERE {
        WINDOW :w1 {
            ?s saref:hasValue ?o .
    ...
    let from_date = new Date(to_date.getTime() - 1800);
    ...
    emitter.on('RStream', async (data: any) => {
        ...
        if(query_registered_time !== null && file_streamer_done_time !== null && first_message_arrival_time !== null && !has_been_written) {
            fs.appendFileSync('query_latency_noagg.csv', `1800,${(file_streamer_done_time - query_registered_time)/1000},${(first_message_arrival_time - file_streamer_done_time)/1000}\n`);
