...
FROM NAMED WINDOW :w1 ON STREAM <${ldes_location}> [RANGE 60 STEP 20]
WHERE {
    WINDOW :w1 {
        ?s saref:hasValue ?o .
    ...
    record_usage('increasing-window-sizes', 'query60sec-bvp-with-agg', 1000);
});
...
websocket.on('message', (message: any) => {
    ...
    if (status === 'stream_reader_ended'){
        ...
        if (file_streamer_done_time !== null && rsp_processing_done_time === null && query_registered_time !== null && isomorphism_done_time !== null && query_sent_time !== null) {
            ...
            fs.appendFileSync('query_latency.csv', `${60},${(isomorphism_done_time - query_sent_time)/1000},${(query_registered_time - isomorphism_done_time)/1000},${(file_streamer_done_time - query_registered_time)/1000},${(rsp_processing_done_time - file_streamer_done_time)/1000}\n`);
        }
    }
