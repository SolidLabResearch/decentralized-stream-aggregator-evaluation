...
async function sixtySecond() {
    ...
    await csvWriter.writeRecords([
        { second: 60, operation: 'find_stream_aggregator', time: (time_two - time_one) / 1000 },
        { second: 60, operation: 'find_relevant_streams', time: (time_three - time_two) / 1000 },
    ]);
    ...
    websocket.once('open', () => {
        let message_object = {
            ...
            `,
            queryId: 'querySixtySecond-with-aggregator'
        };
        websocket.send(JSON.stringify(message_object));
        record_usage('evaluation-1-increasing-window-sizes', 'querySixtySecond-with-aggregator', 1000);
    });
