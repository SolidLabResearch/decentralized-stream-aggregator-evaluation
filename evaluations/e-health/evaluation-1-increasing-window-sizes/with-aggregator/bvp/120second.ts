...
async function hundredtwentySecond() {
    ...
    await csvWriter.writeRecords([
        { second: 120, operation: 'find_stream_aggregator', time: (time_two - time_one) / 1000 },
        { second: 120, operation: 'find_relevant_streams', time: (time_three - time_two) / 1000 },
    ]);
    ...
    websocket.once('open', () => {
        let message_object = {
            ...
            FROM NAMED WINDOW :w1 ON STREAM <${stream}> [RANGE 120 STEP 10]
            WHERE {
                WINDOW :w1 {
                    ?s saref:hasValue ?o .
            ...
            `,
            queryId: 'queryhundredtwentySecond-with-aggregator'
        };
        websocket.send(JSON.stringify(message_object));
        record_usage('evaluation-1-increasing-window-sizes', 'queryhundredtwentySecond-with-aggregator', 1000);
    });
