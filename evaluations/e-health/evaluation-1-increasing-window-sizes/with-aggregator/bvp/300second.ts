...
async function threehundredSecond() {
    ...
    await csvWriter.writeRecords([
        { second: 300, operation: 'find_stream_aggregator', time: (time_two - time_one) / 1000 },
        { second: 300, operation: 'find_relevant_streams', time: (time_three - time_two) / 1000 },
    ]);
    ...
    websocket.once('open', () => {
        let message_object = {
            ...
            FROM NAMED WINDOW :w1 ON STREAM <${stream}> [RANGE 300 STEP 10]
            WHERE {
                WINDOW :w1 {
                    ?s saref:hasValue ?o .
            ...
            `,
            queryId: 'querythreehundredSecond-with-aggregator'
        };
